...
def test_already_running_false_for_live_pid_without_socket(tmp_home):
    ...
    # test process's own pid (definitely alive, definitely not a mboardd, and
    # with no listener on MBOARD_SOCKET). pid_alive() alone would say "running",
...
def test_daemon_spawn_ping_and_sigterm_cleanup(tmp_home):
    ...
    if len(child_env["MBOARD_SOCKET"]) > 103:
        import hashlib
        _h = hashlib.md5(config.home().encode()).hexdigest()[:8]
        child_env["MBOARD_SOCKET"] = "/tmp/mbx-" + _h + ".sock"
    ...
    try:
        sock_path = child_env["MBOARD_SOCKET"]
        ...
        _saved_sock = os.environ.get("MBOARD_SOCKET")
        os.environ["MBOARD_SOCKET"] = sock_path
        try:
            assert daemon.already_running() is True
        finally:
            if _saved_sock is None:
                os.environ.pop("MBOARD_SOCKET", None)
            else:
                os.environ["MBOARD_SOCKET"] = _saved_sock
