...
def test_pre_tool_use_fail_open_when_daemon_unreachable(tmp_home):
    # Safety-critical: a down/broken mboard must NEVER block a real edit.
    # Point MBOARD_SOCKET at an unreachable, unbindable socket path: an
    ...
    env = dict(os.environ)
    env["MBOARD_SOCKET"] = unreachable_sock
    env["PYTHONPATH"] = SRC_DIR + os.pathsep + env.get("PYTHONPATH", "")
    proc = subprocess.run(
    ...
    assert proc.stdout.decode("utf-8").strip() == "", (
        "fail-open violated: hook produced output when mboard was unreachable: "
