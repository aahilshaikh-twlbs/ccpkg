# Handoff Extension — Mboard Coordination

This document captures the exact additions required by **Contract §15** to extend the
`/handoff` skill so that multi-session fan-outs are safe and coordinated via Mboard.
...
### Edit A — Add the "Mboard board" bullet to §0 Orientation in the template
