# Mboard — Frozen Implementation Contract
...
```
~/Documents/Code/mboard/            # dev repo (git)
  pyproject.toml
  ...
  src/mboard/
    ...
    client.py
    cli.py
  bin/mboard                        # shell shim -> python -m mboard.cli
  hooks/
    session_start.py
    pre_tool_use.py
    post_tool_use.py
  ...
  tests/
    ...
    test_hooks.py
    test_e2e.py
  docs/specs/2026-06-03-mboard-design.md
  docs/internal/contract.md          # this file
```

Runtime/install target: `~/.claude/mboard/` (symlinks to repo `src`/`bin`/`hooks`,
plus runtime `state/`, `mboardd.sock`, `mboardd.pid`, `mboardd.log`).
...
def socket_path() -> str: return os.environ.get("MBOARD_SOCKET", os.path.join(home(), "mboardd.sock"))
def pidfile() -> str:     return os.path.join(home(), "mboardd.pid")
def logfile() -> str:     return os.path.join(home(), "mboardd.log")
...
## 7. engine.py — MboardEngine (the core; pure, clock-injected)
...
def join(self, session_id, label, cwd, team=None, member=None, board_name=None) -> dict
    ...
    # session, body "<label> joined <checkout|board> — N now active; coordinate via mboard").
...
def main() -> int
    ...
    # engine.load(). create MboardServer(engine, socket_path()). write_pidfile().
    # install SIGTERM/SIGINT -> server.shutdown()+cleanup(remove socket+pidfile)+exit.
    # serve_forever().
# `python -m mboard.daemon` calls main().
```
...
def ensure_running() -> None
    ...
    #   subprocess.Popen([sys.executable,"-m","mboard.daemon"], stdout/stderr->logfile,
...
## 12. cli.py — `mboard <subcommand>`

`python -m mboard.cli`. Resolve session id: `--session` flag, else
`$MBOARD_SESSION_ID`, else print error "no session id (run inside a Claude session)".
...
- **session_start.py**: read `session_id, cwd, source`. `board_name = os.environ.get("MBOARD_BOARD")`. `label = os.environ.get("MBOARD_LABEL") or basename(cwd)+"-"+sid[:4]`. call `client.request("join", {"session_id":sid,"label":label,"cwd":cwd,"board_name":board_name})`. If `$CLAUDE_ENV_FILE` set: append `export MBOARD_SESSION_ID=<sid>\n` and `export MBOARD_LABEL=<label>\n`. If response data has non-empty `colocated`: print a plain-text line (SessionStart stdout → context): "🤝 Sharing <board> with: <labels>. File claims are auto-enforced; use `mboard ps|claims|send|inbox` to coordinate." exit 0.
...
- **pre_tool_use.py**: read `session_id, cwd, tool_name, tool_input`. If `tool_name` not in `WRITE_TOOLS` → exit 0. `fp = tool_input.get("file_path") or tool_input.get("notebook_path")`; if missing → exit 0. `abs_path = os.path.abspath(os.path.join(cwd, fp))`. call `check_write`. data.decision:
  - "deny": print JSON `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"🔒 <holder> holds <abs_path> (active <since>s ago)<; note>. Coordinate: `mboard request-release <abs_path>` or work elsewhere."}}`; exit 0.
  - "warn": print JSON `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow","additionalContext":"⚠️ <holder> has a STALE claim on <abs_path> (<stale>s). Proceeding, but if they're still working this will collide. `mboard seize <abs_path>` to take ownership."}}`; exit 0.
  - "allow"/else: exit 0 (no output; auto-claim already recorded by daemon).

- **post_tool_use.py**: read `session_id`. call `heartbeat`; call `poll_inbox`. If messages: build text "📬 Mboard: " + one line per msg ("[kind] from <from_label>: <body>"). print JSON `{"hookSpecificOutput":{"hookEventName":"PostToolUse","additionalContext":"<text>"}}`. exit 0.
...
```python
def main():
    # 1) mkdir ~/.claude/mboard; symlink repo src->{home}/mboard_pkg? Simpler:
    #    write {home}/mboard (the bin shim) and ensure PYTHONPATH includes repo src.
    #    Concretely: symlink repo bin/mboard -> {home}/mboard; symlink hooks dir.
    # 2) Read ~/.claude/settings.json; merge mboard hook entries WITHOUT removing the
    #    existing SUPERSET notify hooks (append into each event's array). Idempotent:
    #    skip if a hook command already contains "mboard/hooks". Write back atomically.
...
Edit `~/.claude/commands/handoff.md`: add a "Mboard coordination (multi-session)"
...
- For each segment, lists **bootstrap commands** the receiving session runs first:
  `export MBOARD_BOARD=<slug>` (so SessionStart joins it), then
  `mboard claim <suggested globs> --note "<segment>"` to stake territory, and a note
  to use `mboard ps|inbox|send` for dependency coordination.
...
- Fixture `engine` → `MboardEngine(tmp_path, now_fn=clock)` with a mutable fake clock
  (`clock.t` advanced in tests) for deterministic liveness.
- Fixture `tmp_home(monkeypatch, tmp_path)` → sets `MBOARD_HOME` + `MBOARD_SOCKET`
...
11. server.py (MboardServer) + test_server (round-trip over temp socket).
...
15. bin/mboard shim (+ executable bit) + smoke test.
