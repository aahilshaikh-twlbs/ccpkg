# Mailbox — write concurrency model
...
The mailbox is a single long-lived **daemon** (`mailbox.daemon`) listening on a Unix
domain socket (`~/.claude/mailbox/mailboxd.sock`). Every participant — an agent, a
...
- `client.py` is **socket-only**: `_connect()` opens an `AF_UNIX` stream to
  ...
  **autospawns** one (`python -m mailbox.daemon`) and waits for it to come up — it does
...
```python
# server.py — _MailboxStreamHandler.handle
with self.server.engine_lock:            # threading.Lock(), created in MailboxServer.__init__
...
| Concern | File / symbol |
|---|---|
| Client transport (socket-only, autospawn) | `src/mailbox/client.py` — `_connect`, `request` |
| Request loop + the single-writer lock | `src/mailbox/server.py` — `_MailboxStreamHandler.handle`, `MailboxServer.engine_lock` |
| Mutations (only writer of board state) | `src/mailbox/engine.py` → `store.atomic_write_json` |
| Atomic persistence | `src/mailbox/store.py` — `atomic_write_json` |
| Daemon entrypoint / pidfile | `src/mailbox/daemon.py` |
